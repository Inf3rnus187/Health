...
| POST | `/foods` | `write:measurements` | JSON `FoodIn` (name, brand, aliases, package_g, unit_name, unit_g, portion_g, per_100g, note, source, barcode, product_info) | Add a food: name, brand, aliases, package weight, values per 100 g. |
| POST | `/foods/read-label` | `write:measurements` | form `file` | Read a pack or its nutrition table with the vision model. |
| POST | `/foods/refresh` | `write:measurements` | — | Read again every sheet that has a barcode (about a minute at most). |
...
| PUT | `/foods/{food_id}` | `write:measurements` | `food_id`, JSON `FoodIn` (name, brand, aliases, package_g, unit_name, unit_g, portion_g, per_100g, note, source, barcode, product_info) | Change a food (every field; the meals already read keep theirs). |
...
| POST | `/foods/{food_id}/refresh` | `write:measurements` | `food_id` | Read one sheet's Open Food Facts page again, by its barcode. |
...
| GET | `/system/version` | Tout jeton | — | The version the hub runs (any signed-in user; no host detail). |
